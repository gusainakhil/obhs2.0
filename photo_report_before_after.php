...
<body class="bg-slate-50">
    ...
    <div class="lg:ml-64 min-h-screen">
        ...
        <main class="p-4 lg:p-6">
            ...
            <!-- Page Header -->
            <div class="mb-6">
                <h2 class="text-2xl font-bold text-slate-800">Photo Report - Before & After</h2>
                <p class="text-sm text-slate-600 mt-1">View coach-wise cleaning photos - Before and After comparison</p>
            </div>
                ...
                <div class="coach-container">
                    ...
                    <div class="time-sections">
                        ...
                        <div class="time-section">
                            <div class="time-header before-header">BEFORE CLEANING</div>
                            <div class="photo-grid">
                                <?php
                                $before_query = "SELECT * FROM base_photo_report
                                               WHERE grade = ?
                                               AND coach_no = ?
                                               AND train_no = ?
                                               AND station_id = ?
                                               AND DATE(created_at) = ?
                                               AND time_of_cleaning = 'Before'
                                               ORDER BY created_at
                                               LIMIT 6";
                                $stmt_before = $mysqli->prepare($before_query);
                                $stmt_before->bind_param("sssss", $selected_grade, $coach_no, $selected_train, $station_id, $date_only);
                                $stmt_before->execute();
                                $before_result = $stmt_before->get_result();
                        ...
                        <div class="time-section">
                            <div class="time-header after-header">AFTER CLEANING</div>
                            <div class="photo-grid">
                                <?php
                                $after_query = "SELECT * FROM base_photo_report
                                               WHERE grade = ?
                                               AND coach_no = ?
                                               AND train_no = ?
                                               AND station_id = ?
                                               AND DATE(created_at) = ?
                                               AND time_of_cleaning = 'After'
                                               ORDER BY created_at
                                               LIMIT 6";
                                $stmt_after = $mysqli->prepare($after_query);
                                $stmt_after->bind_param("sssss", $selected_grade, $coach_no, $selected_train, $station_id, $date_only);
                                $stmt_after->execute();
                                $after_result = $stmt_after->get_result();
