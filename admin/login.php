...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($inputUser === '' || $inputPass === '') {
    ...
    // Prepared statement to fetch user by username or email
    $sql = "SELECT `user_id`, `organisation_name`, `username`, `mobile`, `email`, `station_id`, `password`, `start_date`, `end_date`, `type`, `status`, `created_at` FROM `OBHS_users` WHERE (`username` = ? OR `email` = ?) AND `type` = 1 LIMIT 1";
    ...
    if ($stmt = mysqli_prepare($conn, $sql)) {
      ...
      if ($result && $row = mysqli_fetch_assoc($result)) {
        ...
        $ok = false;
        // If passwords were hashed using password_hash()
        if (function_exists('password_verify') && password_verify($inputPass, $stored)) {
          $ok = true;
...
<html lang="en">
  ...
  <head>
    ...
    <title>Beatle analytics | Login Page</title>
  ...
  <body class="login-page bg-body-secondary">
    <div class="login-box">
      <div class="login-logo">
        <a href="index.php"><b>Beatle Analytics</b> OBHS</a>
      </div>
      <!-- /.login-logo -->
      <div class="card">
