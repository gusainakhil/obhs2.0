<?php
    ...
    $sheet->setCellValue('I'.$row, 'Coach Type:');
    $sheet->setCellValue('J'.$row, $coach_type);
    $row += 2;

    // Table header (adjust columns as per your table)
    // Find the max number of feedbacks for any passenger to set column count
    $feedbackData = getAllFeedbackDetailsWithoutGrade($train_no, $from_date, $to_date, $coach_type);
    $max_feedbacks = 0;
    ...
    $headers = array_merge($headers, [ 'Train No']);
    ...
    header('Content-Disposition: attachment;filename="without-grade-all-feedback-detail-report.xlsx"');
