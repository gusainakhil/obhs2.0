...
<head>
    ...
    <title>Round-Wise Summary Without Grade - <?php echo $station_name ?></title>
        ...
        <main class="p-4 lg:p-6">
                ...
                <div class="filter-row" style="display: flex; flex-wrap: nowrap; align-items: flex-end; gap: 10px; overflow-x: auto;">
                    ...
                    <script>
                        ...
                        function exportToExcel() {
                            ...
                            excelData += '<tr><td style="font-weight:bold;">From Date:</td><td colspan="3">' + fromDate + '</td><td style="font-weight:bold;">To Date:</td><td colspan="11">' + toDate + '</td></tr>';
                        ...
                        if (downloadAllButton) downloadAllButton.addEventListener('click', function() {
                            ...
                            window.open('without-grade-download-all-reports-pdf.php?' + params.toString(), '_blank');
                        });
                        
                        const downloadAllExcelButton = document.getElementById('downloadAllExcelButton');
                        if (downloadAllExcelButton) downloadAllExcelButton.addEventListener('click', function() {
                            // Get current filter values
                            const urlParams = new URLSearchParams(window.location.search);
                            const params = new URLSearchParams({
                                from_date: urlParams.get('from_date') || '',
                            ...
                            window.open('without-grade-download-all-reports-excel.php?' + params.toString(), '_blank');
                        });
                    </script>
            ...
            <div class="table-wrapper">
                ...
                <tr>
                    ...
                    $upAchieve = acheived_feedback_without_grade($up, $from_date, $to_date);
                    ...
                    $downAchieve = acheived_feedback_without_grade($down, $from_date, $to_date);
                    ...
                    <tbody>
                        <tr>
                            <td>1</td>
                            <td><a href="<?php echo 'without-grade-train-report.php?' . http_build_query(['train_no' => $up, 'from_date' => $from_date, 'to_date' => $to_date]); ?>"
                                    target="_blank" rel="noopener noreferrer"
                                    class="train-link"><?php echo htmlspecialchars($up); ?></a></td>
                                    
                            <td><?php $trainUpData = get_coach_count($up);
                            echo $trainUpData['ac']; ?> </td>
                            <td><?php $uptrainachivedata = acheived_feedback_without_grade($up, $from_date, $to_date);
                        ...
                        <tr>
                            <td>2</td>
                            <td><a href="<?php echo 'without-grade-train-report.php?' . http_build_query(['train_no' => $down, 'from_date' => $from_date, 'to_date' => $to_date]); ?>"
                                    target="_blank" rel="noopener noreferrer"
                                    class="train-link"><?php echo htmlspecialchars($down); ?></a></td>
                            <td><?php $trainDownData = get_coach_count($down);
                            echo $trainDownData['ac']; ?> </td>
                            <td><?php $downtrainachivedata = acheived_feedback_without_grade($down, $from_date, $to_date);
            ...
            <div class="print-footer" style=" margin-top: 20px; margin-bottom: 20px;">
               Station: <?php echo $station_name; ?> | UP Train: <?php echo $up; ?> | DOWN Train: <?php echo $down; ?> | Report Date: From <?php echo $from_date; ?> To <?php echo $to_date; ?>
            </div>
