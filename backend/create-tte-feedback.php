...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $coach_type = trim($_POST['coach_type'] ?? 'AC');
    ...
    if ($error_message === '') {
        ...
        $sql_passenger = "INSERT INTO OBHS_passenger (id, name, ph_number, pnr_number, seat_no, coach_no, train_no, coach_type, station_id, grade, created ,verified) 
                          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        
        if ($stmt = $mysqli->prepare($sql_passenger)) {
            // Types: id(s), name(s), ph(s), pnr(s), seat(i), coach(s), train(s), coach_type(s), station_id(s), grade(s), created(s), verified(i)
            $stmt->bind_param("ssssissssssi", $passenger_id, $passenger_name, $phone_number, $pnr_number, $seat_no, $coach_no, $train_no, $coach_type, $station_id, $grade, $date, $verified);
    ...
    <div class="container">
        ...
        <div class="content">
            <div class="content-section">
                ...
                <form id="acFeedbackForm" method="POST" action="">
                    <div class="form-row">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <input  type="hidden" name="coach_type" value="TTE" readonly>
                    ...
                    <div class="form-row">
                        ...
                        <table>
                            ...
                            <tbody>
                                <?php
                                // Fetch AC questions from the database
                                $station_id = $_SESSION['station_id'];
                                
                                // Get AC questions
                                $sql_q = "SELECT id, eng_question, hin_question FROM OBHS_questions WHERE type = 'AC' AND station_id = ? ORDER BY id ASC";
                                $ac_questions = [];
                                ...
                                if ($stmt = $mysqli->prepare($sql_q)) {
                                    ...
                                    $stmt->execute();
                                    $result = $stmt->get_result();
                                    $ac_questions = $result->fetch_all(MYSQLI_ASSOC);
                                    $stmt->close();
                                }
                                ...
                                if (count($ac_questions) > 0) {
                                    foreach ($ac_questions as $index => $question) {
                                    ...
                                    echo "<tr><td colspan='2' style='text-align:center; color: #999;'>No AC questions available for this station</td></tr>";
                                }
                                ?>
                            </tbody>
                        </table>
